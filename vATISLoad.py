...
def add_profile_action():
    ...
    tk.Button(add_window, text="Save", command=save_profile).grid(row=2, column=0, columnspan=2)    
...
def delete_profile_window():
    ...
    tk.Label(delete_window, text="Select a Profile to Delete:").grid(row=0, column=0, padx=10, pady=10)
    ...
    def delete_selected_profile():
        ...
        if not selected_profile:
            messagebox.showwarning("Selection Error", "Please select a profile to delete.", delete_window)
            return
        ...
        if selected_profile not in data['facilities']:
            messagebox.showwarning("Delete Error", f"Profile '{selected_profile}' not found in config.", delete_window)
            return
    ...
    delete_button = tk.Button(delete_window, text="Delete", command=delete_selected_profile)
...
def create_ui():
    ...
    profile_listbox.bind('<Double-Button-1>', on_profile_double_click)  # Bind double-click
