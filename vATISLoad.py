...
def delete_profile(profile):
    ...
    if profile in data['facilities']:
        ...
        messagebox.showinfo(
            "Deleted", f"Profile '{profile}' deleted successfully.")
    else:
    ...
    for i in range(len(data['profiles'])):
        ...
        for j in range(len(data['profiles'][i]['composites'])):
            ...
            if len(comp['presets']) == 0 or 'D-ATIS' not in comp['presets'][0]['name']:
                if added_datis == 0:
                    messagebox.showinfo(
                        "Information", "Creating D-ATIS presets...")
    ...
    if added_datis > 0:
        ...
        if save_output:
            with open(config, 'w') as f_out:
                f_out.write(json.dumps(data, indent=4))
            messagebox.showinfo(
                "Saved", "New D-ATIS presets saved successfully.")
        else:
            messagebox.showinfo(
                "Cancelled", "Preset additions were cancelled.")
    else:
...
def run_profile(profile):
    ...
    for ident in AIRPORTS:
        ...
        if tab == -1:
            ...
            messagebox.showinfo(
                "Not Found", f'{ident} NOT FOUND.', parent=parent)
            continue
        ...
        if len(code) == 0 or len(atis[0]) == 0:
            parent.attributes('-topmost', True)
            messagebox.showinfo(
                "No Code", f'{ident.upper()} - UN', parent=parent)
            continue
        ...
        if state == 'CON':
            parent.attributes('-topmost', True)
            messagebox.showinfo(
                "ATIS State", f"ATIS connected for {ident.upper()} - {code}", parent=parent)
        elif state == 'ON':
            parent.attributes('-topmost', True)
            messagebox.showinfo(
                "ATIS State", f"ATIS already connected for {ident.upper()} - OL/{code}", parent=parent)
        else:
            if len(code) > 0:
                parent.attributes('-topmost', True)
                messagebox.showinfo(
                    "ATIS State", f"Unable to connect ATIS for {ident.upper()} - UN/{code}", parent=parent)
            else:
                parent.attributes('-topmost', True)
                messagebox.showinfo(
                    "ATIS State", f"Unable to connect ATIS for {ident.upper()} - UN", parent=parent)
    root.destroy()
...
def add_profile_action():
    ...
    tk.Label(add_window, text="Airports (comma separated):").grid(
        row=1, column=0)
...
def delete_profile_window():
    ...
    def delete_selected_profile():
        ...
        confirm = messagebox.askyesno(
            "Confirm Delete", f"Are you sure you want to delete the profile '{selected_profile}'?"
        )
        if not confirm:
            return
        ...
        config_path = os.getenv('LOCALAPPDATA') + \
            '\\vATIS-4.0\\vATISLoadConfig.json'
        if not os.path.isfile(config_path):
            config_path = os.getenv('LOCALAPPDATA') + \
                '\\vATIS\\vATISLoadConfig.json'
        ...
        if selected_profile not in data['facilities']:
            messagebox.showwarning(
                "Delete Error", f"Profile '{selected_profile}' not found in config.", delete_window)
            return
